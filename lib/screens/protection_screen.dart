...
class ProtectionScreen extends StatefulWidget {
  ...
  final String deviceId; // Added to fetch specific device data

  const ProtectionScreen({
    super.key, 
    required this.sessionCookie,
    this.deviceId = "", // Default empty if not passed
  });
...
class _ProtectionScreenState extends State<ProtectionScreen> with SingleTickerProviderStateMixin {
  ...
  Future<double> _calculateWetnessDuration(String id) async {
    try {
      ...
      if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        List<dynamic> list = [];
        if (jsonResponse is List) list = jsonResponse;
  ...
  Future<void> _fetchDefaultDevice() async {
     try {
      final response = await http.get(
  ...
  void _calculateRisks(double temp, double wetnessHours, double humidity) {
    ...
    setState(() {
      ...
      // Save full objects for Pests too
      _pestRisks = {
  ...
  Map<String, dynamic> _calculatePowderyMildew(double temp, double humidity) {
    if (temp < 10 || temp > 25 || humidity < 70) return {'value': 0, 'status': "Low"};
  ...
  Map<String, dynamic> _calculateBlackRot(double temp, double wetnessHours) {
    if (temp < 20 || temp > 35 || wetnessHours < 4) return {'value': 0, 'status': "Low"};
  ...
  Map<String, dynamic> _getCodlingMothRisk(double degreeDays) {
    ...
    if (degreeDays > 50 && degreeDays <= 250) risk = {'value': 40, 'status': "Medium"};
  ...
  Map<String, dynamic> _getAphidRisk(double temp, double humidity) {
    ...
    if (temp > 18 && temp < 25 && humidity < 70) risk = {'value': 90, 'status': "High"};
  ...
  Map<String, dynamic> _getAppleMaggotRisk(double degreeDays) {
    Map<String, dynamic> risk = {'value': 0, 'status': "Low"};
    if (degreeDays > 900) risk = {'value': 80, 'status': "High"};
  ...
  Map<String, dynamic> _getSpiderMiteRisk(double temp, double humidity) {
    Map<String, dynamic> risk = {'value': 10, 'status': "Low"};
    if (temp > 29 && humidity < 60) risk = {'value': 95, 'status': "High"};
    else if (temp > 25 && humidity < 70) risk = {'value': 60, 'status': "Medium"};
    return risk;
  }

  Map<String, dynamic> _getSanJoseScaleRisk(double degreeDays) {
    Map<String, dynamic> risk = {'value': 0, 'status': "Low"};
    if (degreeDays > 400 && degreeDays < 600) risk = {'value': 90, 'status': "High"};
                ...
                Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Icon(LucideIcons.sprout, color: riskColor, size: 28),
                    ),
                  ],
                ),
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(LucideIcons.bot, size: 20, color: Color(0xFF2563EB)),
                    ),
